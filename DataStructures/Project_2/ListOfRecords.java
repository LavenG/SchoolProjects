...
public class ListOfRecords {
	...
	public void sort(String key) {
		...
		list.sortQuadratic(key);
	}

	/**
	 * Sorts list with the specified key by calling the Merge sort method of
	 * MyArrayList on it
	 * 
	 * @param key
	 *            a String that specifies the desired sorting key
	...
	/**
	 * Loops through an list of Record objects and checks if it is sorted in
	 * ascending order depending on the given key by calling the isSorted method
	 * of MyArrayList
	 * 
	 * @param key
	 *            a String that specifies the desired sorting key
	...
	public boolean isSorted(String key) {
		// If the list is empty then it's already sorted so do nothing
		if (list.size() == 0) {
			return true;
		}
		// Otherwise call the isSorted method on the list with the provided key
		// to determine if the list is sorted
		boolean isSorted = list.isSorted(key);
		return isSorted;
	}
	...
	public MyArrayList findByDate(int numOfDates) {
		...
		for (int i = 0; i < numOfDates; i++) {
			// find the index of the current earliest date
			int currentEarliestDateIndex = results.size();
			// get the inspection date at the current earliest date
